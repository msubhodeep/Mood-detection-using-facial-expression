...
train_data, train_labels = np.arange(10).reshape((5, 2)), range(5)
...
nbc.train(train_data_mat, cv2.ml.ROW_SAMPLE, train_labels_mat)
nbc.save('nbc_data.dat')
t_train_end = time.time()
print ('Time taken to train {}'.format(t_train_end - t_train_start))



results = nbc.predict(train_data_mat)
